...
// NOTE(birdai): the upstream `#[cfg(rust_nightly)] impl Allocative for !` was removed.
// Recent nightlies consider `!` and `Infallible` overlapping, so the two impls conflict
// (E0119) and any nightly build of the workspace fails inside this third-party crate.
// Nothing needs the `!` impl specifically: stable toolchains never compiled it (the cfg is
// only set by this crate's build.rs on nightly), and the whole workspace — including the
// Move package system that pulls allocative in — builds and passes tests without it.
