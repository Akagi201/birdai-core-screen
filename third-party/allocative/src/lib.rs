...
// NOTE(birdai): upstream also gated `feature(never_type)` here for the `!` impl removed
// in `impls/std/unsorted.rs`; nothing else uses the never type, and stable never enabled it.
